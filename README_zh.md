...
│   ├── b8_q200/              #   1008, batch=8, queries=200（預設）
│   ├── b8_q50/               #   1008, batch=8, queries=50（VRAM 優化版）
│   ├── b8_q50_r840/          #   840 解析度
│   ├── b8_q50_r672/          #   672 解析度
│   ├── b8_q50_r560/          #   560 解析度
│   ├── b8_q50_r448/          #   448 解析度（最快、最省 VRAM）
│   └── tokenizer.json
...
### 單攝影機（`infer.py`，4 classes: 3 text + 1 both，Q50）

| 解析度 | 引擎變體 | 平均耗時/幀 | FPS | VRAM |
|--------|---------|------------|-----|------|
...
### 多攝影機（`infer_multi.py`，8 路同時，3 支影片循環，Q50）

| 解析度 | 平均耗時/輪 | 每路攝影機 | 每路 FPS | 總吞吐量 | Buffers |
|--------|-----------|----------|---------|---------|---------|
| 1008 (Q200) | 426 ms | 53 ms | 2.3 | 18.8 FPS | 1,998 MB |
| 1008 (Q50) | 425 ms | 53 ms | 2.3 | 18.8 FPS | 1,238 MB |
| 840 | 446 ms | 55.8 ms | 2.2 | 17.9 FPS | — |
| **672** | **253 ms** | **31.6 ms** | **4.0** | **31.7 FPS** | — |
| **560** | **147 ms** | **18.4 ms** | **6.8** | **54.3 FPS** | **383 MB** |
| **448** | **132 ms** | **16.4 ms** | **7.6** | **60.8 FPS** | **246 MB** |

> **8 GB 部署建議**：560（6.8 FPS/cam）或 448（7.6 FPS/cam）。
> 840 需要 12.9 GB — 8 GB GPU 不適用。
